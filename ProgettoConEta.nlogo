globals [ max-sheep old-age-death]  ; don't let the sheep population grow too large

; Sheep and wolves are both breeds of turtles
breed [ sheep a-sheep ]  ; sheep is its own plural, so we use "a-sheep" as the singular
breed [ wolves wolf ]

turtles-own [ energy juvenile old age steps ]       ; both wolves and sheep have energy
...
to setup
  ...
  [
    ...
    set energy random (2 * sheep-gain-from-food) + 1  ;;;; aggiunto +1 per evitare che muoia subito
    set juvenile sheep-breeding-period-start          ;;;;;;;;;
    set old (juvenile + sheep-breeding-period)        ;;;;;;;;
    set age random ( old + 1 )              ;;; age randomly between 0 and old
    set steps 0
  ...
  [
    ...
    set energy random (2 * wolf-gain-from-food) + 1 ;;;; idem come sopra
    set juvenile wolf-breeding-period-start         ;;;;;;;;
    set old (juvenile + wolf-breeding-period)       ;;;;;;;
    set age random ( old + 1 )             ;;;;
    set steps 0
    setxy random-xcor random-ycor
  ]
  display-labels
...
to go
  ...
  ask sheep [
    move
    set steps steps + 1      ;;;;; counts number of steps ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    if steps mod agefy = 0[   ;;;; counts how much the sheep age ;;;;;
      set age (age + 1)
    ]
...
to reproduce-sheep  ; sheep procedure
  ...
  if age > juvenile and age < old [
    if energy > sheepBreeding[              ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
to reproduce-wolves  ; wolf procedure
  ...
  if age > juvenile and age < old [
    if energy > wolfBreeding [              ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
      set energy (energy / 2)               ; divide energy between parent and offspring
      hatch 1 [ rt random-float 360 fd 1
        set age 0
        set steps 0 ]  ; hatch an offspring and move it forward 1 step
     ]
    ;]
  ]
end
...
to death  ; turtle procedure (i.e. both wolf and sheep procedure)
  ...
  if energy < 0 [ die ]
  if age > ( old + (old / 2) ) [
    set old-age-death old-age-death + 1
    die
  ] ; when age = 100, die
end
...
; 100 sheep 27 wolves 50 energy both 25 energy gain for wolf 4 for sheep =equiliblium
